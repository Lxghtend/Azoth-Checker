...
async def crown_shop():
    crownshop = pyautogui.locateCenterOnScreen("crownshop.png")

    if crownshop != '':
        pyautogui.click(crown_shop)

async def main_checker(p, total_azoth_stacks):
    await p.mouse_handler.deactivate_mouseless()
    await asyncio.sleep(5)
    #await crown_shop()

    while not await is_visible_by_path(p.root_window, spellbook):
...
async def startup():
    ...
    Notfaj - helpin as always
    Major - helped with a silly little error that should have been obvious
              """) 
...
async def new_logic(p):
    ...
    for i in range(6):
        ...
        if wizard == wizards[time]:
            ...
            await asyncio.sleep(8)
            await main_checker(p, total_azoth_stacks)
            time = time + 1
    subprocess.call(f"taskkill /F /PID {p.process_id}",stdout=subprocess.DEVNULL) #kills the current wizard client
